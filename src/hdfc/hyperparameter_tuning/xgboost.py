...
logger = logging.getLogger('xgboost_classifier')
logger.setLevel(logging.DEBUG)

console_handler = logging.StreamHandler()
file_handler = logging.FileHandler(os.path.join(log_dir, 'xgboost_classifier.log'))
...
def tune_model():
    ...
    param_grid = params["xgboost"]

    tscv = TimeSeriesSplit(n_splits=5)
    
    mlflow.set_tracking_uri("http://localhost:5001")
    mlflow.set_experiment("XGBoostClassifier")
    ...
    with mlflow.start_run():
        ...
        joblib.dump(best_model, os.path.join(model_path, "xgboost.pkl"))
